...
cells.append(new_code_cell(
    ...
    "    legend=False,  # per-block Phase headers already name each group; legend is redundant\n"
...
cells.append(new_code_cell(
    ...
    "    legend=False,  # the two block headers already name the Sagas\n"
...
cells.append(new_markdown_cell(
    "## Supporting cut 1b — AVERAGE gross per film, by Phase\n\n"
    "The total-gross bars above have a confound: Phase 3 earned the most partly *because it has the\n"
    "most films* (11, vs 6 for most Phases). To compare Phases fairly we need **average worldwide\n"
    "gross per film** — how well the typical film in each Phase performed, independent of how many\n"
    "there were.\n\n"
    "This reframes the story: Phase 3 wasn't just volume — its films also averaged high. And **Phase\n"
    "6 has the highest average** despite only two films (both Spider-Man / Fantastic Four\n"
    "blockbusters), so its small total is a sample-size story, not a quality one. The film count is\n"
    "shown in each label so the reader can see what the average is built on."
))
cells.append(new_code_cell(
    "phase_avg = con.execute('''\n"
    "    SELECT phase,\n"
    "           ROUND(AVG(worldwide_gross))  AS avg_worldwide,\n"
    "           COUNT(*)                     AS films\n"
    "    FROM mcu_box_office GROUP BY phase, phase_num ORDER BY phase_num\n"
    "''').df()\n"
    ...
    "display(phase_avg[['phase','avg_worldwide','films']])\n"
    "img = single_ranked_bars(\n"
    "    df=phase_avg, category_col='phase', value_col='avg_worldwide',\n"
    "    total_label_col='label', color_col='color',\n"
    "    title='MCU average box office per film, by Phase',\n"
    "    subtitle='Mean lifetime worldwide gross per film, nominal $; controls for how many films each Phase has',\n"
    "    source='The Numbers, retrieved 2026-09-19',\n"
    "    img_width=1600, img_height=900,\n"
    ")\n"
    "display(img)"
...
cells.append(new_markdown_cell(
    ...
    "  candidate hero.\n"
    "- **Headline facts** the treemap makes visual and the bars make precise:\n"
    ...
    "- **Total vs average, side by side (cuts 1 and 1b):** the total-gross bars are confounded by\n"
    "  film count (Phase 3 has 11), so the average-per-film bars are the fair comparison. They show\n"
    "  Phase 3 averaged high *and* had volume; Phase 6 has the top per-film average ($1.49B) off just\n"
    "  two films. Keep both in exploration; social will use whichever the owner picks.\n"
    "- **Secondary angle:** the domestic/international mix by Phase. Worth keeping as a possible\n"
    "  chart, but no strong trend jumped out.\n"
