...
cells.append(new_code_cell(
    ...
    "# Dollar labels drawn INSIDE each segment alongside the % (via segment_label_cols).\n"
    "split['dom_money']  = split['dom'].map(lambda v: f'${v/1e9:.2f}B')\n"
    "split['intl_money'] = split['intl'].map(lambda v: f'${v/1e9:.2f}B')\n"
    "segments = [{'col':'domestic_pct','label':'Domestic (US & Canada)','color':c('teal_dark')},\n"
    "            {'col':'intl_pct','label':'International','color':c('gold')}]\n"
    "seg_money = ['dom_money', 'intl_money']  # parallel to segments; renders '$X.XXB  NN%' per segment\n"
    "img = stacked_100pct_bars(df=split, group_col='phase', segments=segments,\n"
    "    segment_label_cols=seg_money,\n"
    "    title='MCU revenue mix by Phase \u2014 domestic vs international',\n"
    "    subtitle='Worldwide gross split into domestic (US & Canada) vs international; each Phase normalized to 100%',\n"
